...
function initWords() {
    ...
    if (testMode == "time" || testMode == "words") {
        for (let i = 0; i < wordsConfig; i++) {
            let randomWord = words[Math.floor(Math.random() * words.length)];
            if (punctuationMode) {
                wordsList.push(lowerFirstLetter(randomWord));
            } else {
...
function calculateStats() {
    ...
    $.ajax({
        type: "POST",
        url: "${pageContext.request.contextPath}/WpmData", // Replace with the actual URL of your servlet
        data: {
            modeNumber: "2",
            wpmValue: "Math.round(wpm)"
        },
...
function updateTimer() {
    liveWPM(); // This function is not provided in the code you shared
    let percent = ((timeConfig - time) / timeConfig) * 100;
    $("#timer")
        .stop(true, true)
